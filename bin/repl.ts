...
import { handle, newSession, planShown, respondido, seguir, perguntando, retomando, aprovando, confirmandoTarefa } from '../lib/core/session'
...
import { lerEntrada } from '../lib/core/parece-tarefa'
...
function dicaDa(state: SessionState, sugerindo = false): string {
  const texto = ((): string => {
    ...
    if (state.confirmandoTarefa) return 'enter cria mesmo assim · reescreva para trocar'

...
import { esperandoVoce, linhasEspera } from '../../motor/mir/render/rodape.ts'
...
import { janelaDaLista } from '../../motor/mir/render/rodape.ts'
...
import { esperaHumano } from '../../motor/mir/render/phases.ts'

test('uma definicao so de "esperando voce" — rodape e abas concordam', async () => {
  const { abasDe } = await import('../../motor/mir/render/board.ts')
  const cards = [
...
import { esperandoEmOutrosProjetos, linhaDeOutrosProjetos } from '../../motor/mir/render/rodape.ts'

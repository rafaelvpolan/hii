...
const { anexarEvento, eventosDoCard } = await import('../../motor/euclides/eventos.ts')
const { retomarAoIniciar, faseInterrompida } = await import('../../motor/euclides/recuperar.ts')

// A auditoria (Pluto + Crivo) achou que motor/euclides/recuperar.ts era um modulo
...
test('INVARIANTE runner.ts chama retomarAoIniciar no arranque — senao o modulo e orfao', async () => {
  ...
  expect(fonte).toContain("from './motor/euclides/recuperar.ts'")
...
test('o laco de ajuste de URL passou a registrar tentativa no diario', async () => {
  const { subirUrlComAjuste } = await import('../../motor/ciclo/reprise/url-ajuste.ts')
  let subidas = 0
...
test('fase EXTERNA aberta nao e mascarada por uma fase interna que fechou depois', async () => {
  const { fasesInterrompidas } = await import('../../motor/euclides/recuperar.ts')

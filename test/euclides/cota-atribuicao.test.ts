...
test('REGRESSAO classeDeFalha sobrevive ao ida-e-volta do ledger — sem isso a atribuicao de cota e cega', async () => {
  const { registrarChamada, chamadasDaSessao, agregarPorIa } = await import('../../motor/euclides/ias-da-sessao.ts')
...
test('classe de falha desconhecida no ledger nao vira classe valida', async () => {
  const { registrarChamada, chamadasDaSessao } = await import('../../motor/euclides/ias-da-sessao.ts')

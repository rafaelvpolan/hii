...
import type { ChamadaDeIa, PapelDeChamada } from '../../motor/cordel/tipos.ts'
...
test('o id curto e deterministico — mesma sessao, mesmo id', async () => {
  const { idCurto } = await import('../../motor/euclides/ias-da-sessao.ts')
...
test('O CASO DO GATED STEP: duas IAs no mesmo passo aparecem separadas, nao somadas', async () => {
  const { agregarPorIa } = await import('../../motor/euclides/ias-da-sessao.ts')
...
test('troca de provedor DENTRO do mesmo papel e evento; por papel diferente nao e', async () => {
  const { trocasDeProvedor } = await import('../../motor/euclides/ias-da-sessao.ts')
...
test('sessao de conversa e separada da sessao de card', async () => {
  const { sessaoParaChamada } = await import('../../motor/euclides/tesouro/confianca.ts')
...
test('o finish refresca as IAs da run, sem perder o que ja estava', async () => {
  const m = await import('../../motor/euclides/ias-da-sessao.ts')
  const { writeRun, updateRunSteps } = await import('../../motor/euclides/registros.ts')
  const sessao = m.abrirSessao('010')
  m.registrarChamada(sessao, chamada({ papel: 'implement' }))
  const rec = writeRun('010', {
  ...
  const { readRunSteps } = await import('../../motor/euclides/registros.ts')

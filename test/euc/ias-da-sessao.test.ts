...
import type { ChamadaDeIa, PapelDeChamada } from '../../motor/cdl/tipos.ts'
...
beforeEach(async () => {
  ...
  const { esquecerSessoes } = await import('../../motor/euc/ias-da-sessao.ts')
  esquecerSessoes()
})
...
test('a sessao de um card e estavel entre chamadas, e uma nova execucao abre outra', async () => {
  const m = await import('../../motor/euc/ias-da-sessao.ts')
...
test('o id curto e deterministico — mesma sessao, mesmo id', async () => {
  const { idCurto } = await import('../../motor/euc/ias-da-sessao.ts')
...
test('O CASO DO GATED STEP: duas IAs no mesmo passo aparecem separadas, nao somadas', async () => {
  const { agregarPorIa } = await import('../../motor/euc/ias-da-sessao.ts')
...
test('troca de provedor DENTRO do mesmo papel e evento; por papel diferente nao e', async () => {
  const { trocasDeProvedor } = await import('../../motor/euc/ias-da-sessao.ts')
...
test('o finish refresca as IAs da run, sem perder o que ja estava', async () => {
  ...
  const { readRunSteps } = await import('../../motor/euc/registros.ts')

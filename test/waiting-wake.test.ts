...
let saudavel = true
let atrasoMs = 0
mock.module('../lib/ai/health-probe', () => ({
  probeProviderHealth: (): Promise<boolean> => new Promise(resolve => setTimeout(() => resolve(saudavel), atrasoMs)),
}))
...
test('REGRESSAO: chamadas concorrentes de wakeDueWaiting nao processam o mesmo card em dobro (guarda de reentrancia)', async () => {
  ...
  atrasoMs = 30
  const id = waitingCard(-1000, '0')
  await Promise.all([wakeDueWaiting(), wakeDueWaiting()])
  atrasoMs = 0

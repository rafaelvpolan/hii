...
import { readPackageJson } from '../../motor/cdl/bss/detectar.ts'
import { readContract } from '../../motor/cdl/bss/armazenar.ts'
import { esquecerAvisosDeArquivo } from '../../motor/cdl/ali/aviso.ts'
...
test('ia.json CORROMPIDO avisa, dizendo que as preferencias NAO foram aplicadas', async () => {
  const { preferencias } = await import('../../motor/tmd/preferencias.ts')
...
test('.hii/config.json CORROMPIDO nao vira "sem preferencia declarada"', async () => {
  const { lerProjectConfig } = await import('../../motor/cdl/ali/home.ts')
  mkdirSync(join(dir, '.hii'), { recursive: true })

...
const { SECOES_DA_MATRIZ, arquivoDaMatriz, criarMatriz } = await import('../../motor/niemeyer/lucio/matriz-entendimento.ts')
...
async function corpoDeApprovePlan(): Promise<string> {
  const fonte = await lerArquivo('motor/mirante/acoes.ts')

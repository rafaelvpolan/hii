...
const M = await import('../../motor/quilombo/cartorio/merge.ts')
const { anexarEvento, eventosDoCard, cardFechado } = await import('../../motor/euclides/eventos.ts')
const C = await import('../../motor/cascudo/freire/candidatos.ts')
const { faseInterrompida } = await import('../../motor/euclides/recuperar.ts')
...
test('INVARIANTE checkMerged chama aoMergear no ramo do merge', async () => {
  const fonte = await lerArquivo('motor/quilombo/cartorio/merge.ts')

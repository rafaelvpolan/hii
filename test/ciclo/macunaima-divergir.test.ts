...
} from '../../motor/ciclo/macunaima/divergir.ts'
import type { Ramo, SaidaDeRamo } from '../../motor/ciclo/macunaima/divergir.ts'
import { lerEnquadramentos } from '../../motor/ciclo/macunaima/enquadramentos.ts'
...
test('TETO POR RAMO chega ao ramo — numero calculado e nao aplicado nao e teto', async () => {
  const { montarRamos, despacharDivergencia, relatoDeEstouro } = await import('../../motor/ciclo/macunaima/divergir.ts')
...
test('sem teto por ramo, ninguem e acusado de estourar — zero significa "nao ha teto", nao "teto zero"', async () => {
  const { ramosQueEstouraram } = await import('../../motor/ciclo/macunaima/divergir.ts')

...
import { escolherReparador, reparadoresRegistrados } from '../../motor/ciclo/reprise/reparadores/index.ts'
...
test('INVARIANTE o portao de build consulta o reparador de dominio — senao isto e codigo morto', async () => {
  const fonte = await lerArquivo('motor/ciclo/crivo/portoes-de-fecho.ts')
...
const { cercarSaida } = await import('../../motor/ciclo/reprise/reparadores/tipos.ts')
...
test('os dois construtores de instrucao usam a cerca — o generico e o de dominio', async () => {
  const generico = await lerArquivo('motor/ciclo/crivo/portoes-de-fecho.ts')

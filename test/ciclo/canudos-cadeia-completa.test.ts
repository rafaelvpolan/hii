...
function comTelaRenderizada(card: string): string {
  const tela = Canudos.telaDoCard(card)
...
test('ELO 1 a referencia anexada na conversa migra para a pasta do CARD', () => {
  ...
  expect(Canudos.referenciasDoCard('101').length).toBe(1)
})
...
test('ELO 2 o pack frontend-web satisfaz o gatilho de dominio para um diff de .vue', () => {
  ...
  expect(Canudos.gauntletVale(packs).vale).toBe(true)
})

test('ELO 3 com o gauntlet LIGADO, referencia no card e dominio de front, o modo escolhido e gauntlet', () => {
  const packs = [...new Set(A.skillsPara('avaliador', { arquivos: ['src/Home.vue'], deps: ['vue'] }, ACERVO).map(s => s.pack))]
  const m = Canudos.modoDoCrivo({ packs, referencias: Canudos.referenciasDoCard('101'), ativado: true })
...
test('ELO 4 a comparacao cega entrega os dois caminhos e NAO diz qual e o do motor', () => {
  ...
  const referencias = Canudos.referenciasDoCard('101')
  const candidatos = [
  ...
  const cega = Canudos.cegar(candidatos, '101')
  const texto = Canudos.renderizarComparacao(cega)
...
test('CADEIA QUEBRA no elo certo: sem tela renderizada, o card cai no criterio escrito', () => {
  ...
  expect(Canudos.referenciasDoCard('102').length).toBe(1)
  expect(existsSync(Canudos.telaDoCard('102')), 'card sem fase de url nao tem tela').toBe(false)
  ...
  const packs = [...new Set(A.skillsPara('avaliador', { arquivos: ['src/Home.vue'], deps: ['vue'] }, ACERVO).map(s => s.pack))]
  expect(Canudos.modoDoCrivo({ packs, referencias: Canudos.referenciasDoCard('102'), ativado: true }).modo).toBe('gauntlet')
})

test('CADEIA QUEBRA no elo certo: card SEM referencia nao entra em gauntlet mesmo sendo front', () => {
  const packs = [...new Set(A.skillsPara('avaliador', { arquivos: ['src/Home.vue'], deps: ['vue'] }, ACERVO).map(s => s.pack))]
  const m = Canudos.modoDoCrivo({ packs, referencias: Canudos.referenciasDoCard('999-sem-ref'), ativado: true })
...
test('ELO 0 o interruptor: a cadeia inteira montada, mas o gauntlet desligado, cai no criterio escrito', () => {
  ...
  expect(Canudos.referenciasDoCard('101').length).toBe(1)
  expect(Canudos.gauntletVale(packs).vale).toBe(true)
  expect(Canudos.modoDoCrivo({ packs, referencias: Canudos.referenciasDoCard('101') }).modo).toBe('criterio-escrito')
})

...
test('url entra como fonte sem baixar nada na hora', async () => {
  const { anexarNaTarefa } = await import('../../motor/qlb/alf/anexo.ts')
...
test('INTEGRACAO: o que o /ref grava passa inteiro pelo resolveRefs, sem recusa', async () => {
  const { anexarNaTarefa } = await import('../../motor/qlb/alf/anexo.ts')
  const { resolveRefs, refPaths } = await import('../../motor/qlb/alf/refs.ts')
...
test('recusa o que nao e imagem, o que nao existe e o que passa do teto de tamanho', async () => {
  const { anexarNaTarefa } = await import('../../motor/qlb/alf/anexo.ts')
  const { MAX_FILESIZE_BYTES } = await import('../../motor/qlb/alf/download.ts')
...
test('o teto de 8 referencias por tarefa e respeitado', async () => {
  const { anexarNaTarefa } = await import('../../motor/qlb/alf/anexo.ts')
  const { MAX_REFS_POR_TAREFA } = await import('../../motor/euc/estado-em-disco.ts')
...
test('sem tarefa aberta a ref fica na sessao e migra para a tarefa depois', async () => {
  const { anexarNaSessao, fontesDaSessao, migrarRefsDaSessao } = await import('../../motor/qlb/alf/anexo.ts')
  const { readRefSources } = await import('../../motor/qlb/alf/refs.ts')
...
test('migrar preserva as refs que a tarefa ja tinha', async () => {
  const { anexarNaSessao, anexarNaTarefa, migrarRefsDaSessao } = await import('../../motor/qlb/alf/anexo.ts')
...
test('o uso de disco soma por area e acende alerta e teto', async () => {
  const { usoDeDisco, nivelDe } = await import('../../motor/euc/estado-em-disco.ts')
...
test('a limpeza de tmp remove o antigo e preserva a sessao em uso', async () => {
  const { limparTmpAntigo, dirDaSessao } = await import('../../motor/euc/estado-em-disco.ts')

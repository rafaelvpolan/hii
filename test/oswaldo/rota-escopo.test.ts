...
import { caminhosCitados, foraDoEscopo, lerEscopo, SEM_ESCOPO } from '../../motor/oswaldo/rota/escopo.ts'
...
test('INVARIANTE plano e motor leem o escopo do MESMO texto', async () => {
  const plano = await lerArquivo('motor/niemeyer/lucio/plano.ts')
...
test('INVARIANTE a tela do plano passa a checagem de existencia — senao mostra prosa como arquivo', async () => {
  const tela = await lerArquivo('motor/mirante/cli/tela-tarefa.ts')
...
// O primeiro cumprimento (oswaldo/executar.ts) ve so a escrita do implement. Depois dele

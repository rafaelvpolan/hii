...
function registro(id: string): string {
  return join(CARDS, 'runs', `${id}.harness.pid`)
}
...
test('registrarHarness grava pid, papel e iniciadoEm ISO; esquecer so apaga se o pid bate', () => {
  ...
  const lido = JSON.parse(readFileSync(registro(id), 'utf8')) as { pid: number; papel: string; iniciadoEm: string }
  ...
  expect(existsSync(registro(id)), 'pid diferente nao pode apagar o registro de outro').toBe(true)
  vivo.esquecerHarness(id, 4242)
  expect(existsSync(registro(id))).toBe(false)
})
...
test('REGRESSAO halt deixa morto o harness que ignora SIGTERM e registra SIGKILL no diario', async () => {
  ...
  expect(await esperarMorte(pid, 2000), 'o harness tinha de estar morto depois do halt').toBe(true)
  const corpo = readCard(id)?.body ?? ''
  expect(corpo).toContain(`harness pid ${pid} encerrado (SIGKILL)`)
  expect(existsSync(registro(id)), 'registro do harness morto tem de sumir').toBe(false)
}, TEMPO_COM_GIT_MS)
...
test('pid registrado que nao e harness nem roda no worktree (reciclado) NAO e morto', async () => {
  ...
  expect(existsSync(registro(id)), 'registro invalido e descartado').toBe(false)
}, TEMPO_COM_GIT_MS)
...
test('varredura no arranque: pid morto limpa o arquivo; vivo em card HALTED e encerrado; vivo em card ativo fica', async () => {
  ...
  expect(existsSync(registro(morto))).toBe(false)
  expect(v.encerrados.map(e => e.id)).toContain(parado)
  expect(await esperarMorte(pidParado, 2000)).toBe(true)
  ...
  expect(existsSync(registro(ativo))).toBe(true)
}, TEMPO_COM_GIT_MS)
...
test('encerramento do motor mata todo harness registrado e deixa linha no diario', async () => {
  ...
  expect(await esperarMorte(pidA, 2000)).toBe(true)
  expect(await esperarMorte(pidB, 2000)).toBe(true)
  ...
  expect(existsSync(registro(a))).toBe(false)
  expect(existsSync(registro(b))).toBe(false)
}, TEMPO_COM_GIT_MS)

...
// diretorio. Trocar HII_CARDS_DIR dentro dessa janela devolvia a lista do
...
test('REGRESSAO o mesmo vale para o registro de repos, que tinha o mesmo defeito', () => {
  ...
  process.env.HII_REPOS_FILE = join(a, 'repos.json')
  expect(reposRegistrados().map(r => r.name)).toEqual(['org/um'])
  process.env.HII_REPOS_FILE = join(b, 'repos.json')

...
// em test/euclides/tesouro-orcamento.test.ts.
...
const ARQUIVOS_DE_TESTE = readdirSync('test/mirante')
  .filter(n => n.endsWith('.test.ts') && n !== 'mapa-de-comandos.test.ts')
  .map(n => ({ nome: n, fonte: semComentarios(readFileSync(join('test/mirante', n), 'utf8')) }))
...
function exercitado(cmd: string, fonte: string): boolean {
  ...
  // as ~30 linhas `import ... '../../motor/mirante/tui/layout.ts'` casavam com
...
test('INVARIANTE todo comando de COMMANDS tem teste que o exercita', () => {
  ...
  expect(semTeste, 'comando novo sem teste: acrescente cobertura em test/mirante/ antes de listar em COMMANDS').toEqual([])
})
...
test('a varredura NAO casa em CAMINHO DE IMPORT — /layout nao e coberto por tui/layout.ts', () => {
  const soImport = "import { renderFrame } from '../../motor/mirante/tui/layout.ts'"

...
import { renderHelp } from '../../motor/mirante/render/help.ts'
...
test('cobre todos os comandos que a sessao aceita', async () => {
  ...
  // dos dois lados, igual a varredura de test/mirante/mapa-de-comandos.test.ts.
...
test('os atalhos de intake vem do MESMO catalogo do parser, nao de uma lista copiada', async () => {
  const { COMANDOS_MANUAIS } = await import('../../motor/mirante/comandos-manuais.ts')
  const t = renderHelp().join('\n')

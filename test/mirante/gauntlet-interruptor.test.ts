...
import { handle, newSession, COMMANDS, canonico } from '../../motor/mirante/sessao.ts'
import { linhaPropriedades } from '../../motor/mirante/render/rodape.ts'
...
test('desligado por omissao: arquivo de preferencia limpo nao liga o gauntlet', async () => {
  const { gauntletLigado } = await import('../../motor/tomada/preferencias.ts')
  expect(gauntletLigado()).toBe(false)
})

test('/gauntlet on liga, /gauntlet off desliga, e a leitura seguinte ja ve', async () => {
  const { definirGauntlet } = await import('../../motor/mirante/escolher-ia.ts')
  const { gauntletLigado } = await import('../../motor/tomada/preferencias.ts')

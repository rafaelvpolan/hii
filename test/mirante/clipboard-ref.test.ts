...
import type { DepsDeColagem, Rodada } from '../../motor/mirante/clipboard.ts'
...
test('o ambiente de clipboard e escolhido pela maquina, com WSL na frente', async () => {
  const { ambienteDeClipboard } = await import('../../motor/mirante/clipboard.ts')
...
test('cada ambiente tem o comando certo, e o do WSL salva no caminho nativo', async () => {
  const { comandoDeColagem } = await import('../../motor/mirante/clipboard.ts')
...
test('a extensao vem da assinatura do arquivo, nao do que o clipboard diz', async () => {
  const { extensaoPelaAssinatura } = await import('../../motor/mirante/clipboard.ts')
...
test('clipboard sem imagem responde o que fazer, em vez de gravar lixo', async () => {
  const { colarImagem } = await import('../../motor/mirante/clipboard.ts')
...
test('/ref sem argumento lista as referencias e o uso de disco', async () => {
  const { comandoRef } = await import('../../motor/mirante/refs-comando.ts')

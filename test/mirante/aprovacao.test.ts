...
import { renderAprovacao, OPCOES_APROVACAO } from '../../motor/mirante/render/aprovacao.ts'
import { renderPendencia, pendenciaDoStatus } from '../../motor/mirante/render/pendencia.ts'
...
test('REGRESSAO 1 2 3 dentro da tarefa em URL cai na aprovacao, nao no plano do card #001', async () => {
  const { handle, newSession, seguir, sincronizarAprovacao } = await import('../../motor/mirante/sessao.ts')
...
test('quando a maquina ja abriu a url, a pergunta ao humano vira de intencao', async () => {
  const { renderAprovacao } = await import('../../motor/mirante/render/aprovacao.ts')
...
test('card antigo, sem veredito gravado, cai na pergunta tecnica e nao inventa veredito', async () => {
  const { renderAprovacao, verificacaoDoCard } = await import('../../motor/mirante/render/aprovacao.ts')

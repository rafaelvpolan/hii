...
import { subPrompts, anexarSubPrompt } from '../../motor/mirante/instruir.ts'
...
test('REGRESSAO paste multilinha vira UMA instrucao, nao sete', async () => {
  const { instruir, subPrompts } = await import('../../motor/mirante/instruir.ts')
  const { readCard } = await import('../../motor/cordel/store.ts')
  card('022', { status: 'HALTED' })
...
test('umaLinha preserva o conteudo e marca as quebras', async () => {
  const { umaLinha } = await import('../../motor/mirante/instruir.ts')
...
test('texto colado chega INTEIRO na instrucao, nao o marcador', async () => {
  const { newInput, keypress } = await import('../../motor/mirante/tui/input.ts')
  const { marcarCola } = await import('../../motor/mirante/tui/keys.ts')
  const { instruir, subPrompts } = await import('../../motor/mirante/instruir.ts')
  const { readCard } = await import('../../motor/cordel/store.ts')
  const erro = 'deu erro:\nApp.vue:97:7\n95 |\n96 |  <EngineConsole />'
...
test('REGRESSAO instrucao dada antes de executar CHEGA ao objetivo que vira prompt', async () => {
  const { objetivoComInstrucoes, instruir } = await import('../../motor/mirante/instruir.ts')
  const { readCard } = await import('../../motor/cordel/store.ts')
...
test('sem instrucao, o objetivo sai igual ao de antes — sem cabecalho vazio', async () => {
  const { objetivoComInstrucoes } = await import('../../motor/mirante/instruir.ts')
  const { readCard } = await import('../../motor/cordel/store.ts')

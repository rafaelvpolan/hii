...
import { renderSugestoes, prefixoComum, AJUDA_DO_COMANDO } from '../../motor/mirante/render/sugestoes.ts'
import { renderFrame, visibleLen, stripAnsi } from '../../motor/mirante/tui/layout.ts'
...
test('a janela respeita o teto de linhas do terminal — senao o quadro corta e a navegacao morre', async () => {
  const { cabemQuantasSugestoes } = await import('../../motor/mirante/render/sugestoes.ts')
...
test('REGRESSAO: quando o espaco aperta, quem cede e o painel de cima, nao a sugestao', async () => {
  const { renderFrame } = await import('../../motor/mirante/tui/layout.ts')
...
test('as bordas do corte de sugestao: nenhuma, algumas, todas', async () => {
  const { orcamentoDoCorpo } = await import('../../motor/mirante/tui/layout.ts')

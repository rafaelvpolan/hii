...
test('sinaisDeFalha traz as tres classes, sempre — a tabela central em ciclo/ deixou de existir', () => {
  for (const nome of providerNames()) {
...
test('REGRESSAO ciclo/ nao conhece nome de harness — senao harness novo obriga a mexer em ciclo/', async () => {
  const fonte = await lerArquivo('motor/ciclo/reprise/classe-de-falha.ts')
  for (const nome of providerNames()) {
...
test('INVARIANTE nenhum despacho por nome de harness fora de motor/tomada/harness/', () => {
  const culpados: string[] = []
  for (const arquivo of arquivosDoMotor()) {
    if (arquivo.startsWith('motor/tomada/harness/')) continue
...
test('registrar um harness e so somar uma linha: o registro nao guarda nada alem da lista', async () => {
  const fonte = await lerArquivo('motor/tomada/registro.ts')
...
test('harnessSeExistir devolve undefined em vez de lancar — os dois contratos convivem', async () => {
  const { harnessSeExistir } = await import('../../motor/tomada/registro.ts')

...
import { comandosDaIaAtiva } from '../../motor/tomada/mapa/comandos.ts'
import { claudeArgv } from '../../motor/tomada/harness/claude-argv.ts'
import { argv as codexArgv } from '../../motor/tomada/harness/codex.ts'
import { kimiArgv } from '../../motor/tomada/harness/kimi.ts'
import { agentRoles, harnessPorNome, modelFor, providerNames } from '../../motor/tomada/registro.ts'
import {
...
test('timeout (wrapper generico de quilombo/git.ts) vira transient em qualquer harness que passe por ele — so o claude tinha prova disto, e so pelo caminho de stream', async () => {
  fakeBin('claude', '#!/usr/bin/env bash\nexec sleep 30\n')
  fakeBin('codex', '#!/usr/bin/env bash\nexec sleep 30\n')
  fakeBin('kimi', '#!/usr/bin/env bash\nexec sleep 30\n')
  fakeBin('curl', '#!/usr/bin/env bash\nexec sleep 30\n')
...
test('ollama fica de fora de FONTES por decisao — mesmo com claude, codex e kimi TODOS povoados ao mesmo tempo, so ollama devolve lista vazia (autoridade: test/tomada/mapa-comandos.test.ts)', async () => {

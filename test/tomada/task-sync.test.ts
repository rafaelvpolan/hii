...
import { parseIssues } from '../../motor/tomada/ponte/tarefas/github-issues.ts'
import { taskSync, taskSyncName, taskSyncNames } from '../../motor/tomada/ponte/tarefas/registro.ts'
...
test('relatoDeSync mostra as falhas em vez de imprimir so os numeros', async () => {
  const { relatoDeSync } = await import('../../motor/tomada/ponte/tarefas/sync.ts')
...
test('TaskSync.push devolve se o efeito aconteceu AGORA — void nao distinguia nada', async () => {
  const { GithubIssuesSync } = await import('../../motor/tomada/ponte/tarefas/github-issues.ts')

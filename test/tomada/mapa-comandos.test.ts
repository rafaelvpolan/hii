...
beforeEach(() => {
  ...
  guardar('HII_CLAUDE_HOME_DIR', join(home, '.claude'))
  guardar('HII_KIMI_HOME_DIR', join(home, '.kimi-code'))
  guardar('HII_IA_FILE', join(home, 'sem-preferencias.json'))
  guardar('HII_IMPLEMENT_PROVIDER', undefined)
})
...
test('cada ia tem uma cor de marca distinta', async () => {
  const { corDaIa } = await import('../../motor/tomada/mapa/comandos.ts')

...
test('a troca vale sem reiniciar: a leitura seguinte ja ve o novo valor', async () => {
  ...
  const { providerNameFor, modelFor } = await import('../../motor/tomada/registro.ts')
...
test('interpretar entende papel, provedor, modelo e esforco em qualquer ordem', async () => {
  const { interpretar } = await import('../../motor/mirante/escolher-ia.ts')
...
test('sem papel, o ajuste vale para todos', async () => {
  const { interpretar } = await import('../../motor/mirante/escolher-ia.ts')
  const { agentRoles } = await import('../../motor/tomada/registro.ts')
...
test('REGRESSAO esforco vira argumento real do CLI nos DOIS caminhos, nao so enfeite no rodape', async () => {
  const { claudeArgv, FORMATO_JSON, FORMATO_STREAM } = await import('../../motor/tomada/harness/claude-argv.ts')
  const req = {
  ...
  const { argv: codexArgv } = await import('../../motor/tomada/harness/codex.ts')
...
test('o pedido do provedor carrega o campo de esforco', async () => {
  const tipos = await lerArquivo('motor/tomada/tipos.ts')
...
test('a listagem do /ia nao esconde provedor sem papel', async () => {
  const { estadoDaIa } = await import('../../motor/mirante/escolher-ia.ts')
...
test('REGRESSAO rodape e motor leem a MESMA fonte de esforco', async () => {
  const { esforcoAtual } = await import('../../motor/mirante/cli/rodape-tui.ts')
  const { effortFor } = await import('../../motor/tomada/registro.ts')
  const { newSession } = await import('../../motor/mirante/sessao.ts')
  const { ESFORCO_PADRAO } = await import('../../motor/tomada/preferencias.ts')
...
test('REGRESSAO o rodape nao pode chutar um esforco fixo', async () => {
  const fonte = await lerArquivo('motor/mirante/cli/rodape-tui.ts')
...
test('/effort sem argumento lista os niveis', async () => {
  const { definirEsforco } = await import('../../motor/mirante/escolher-ia.ts')
...
test('/effort padrao limpa so o esforco, preservando a ia escolhida', async () => {
  const { definirEsforco, aplicar } = await import('../../motor/mirante/escolher-ia.ts')
  const { effortFor, providerNameFor } = await import('../../motor/tomada/registro.ts')
...
test('modelo em uso entra no catalogo mesmo sem estar na semente', async () => {
  const { definirModelo } = await import('../../motor/mirante/escolher-ia.ts')
  const { modelosDe } = await import('../../motor/tomada/catalogo.ts')
...
test('autocompletar de /ia, /model e /effort oferece as opcoes certas', async () => {
  const { complete } = await import('../../motor/mirante/completar.ts')
  const { providerNames, agentRoles } = await import('../../motor/tomada/registro.ts')
  const { modelosDe } = await import('../../motor/tomada/catalogo.ts')
  const { ESFORCOS } = await import('../../motor/tomada/preferencias.ts')
  const ctx = {

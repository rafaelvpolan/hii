...
import { KimiProvider, KIMI_CAPACIDADES, kimiArgv } from '../../motor/tomada/harness/kimi.ts'
import { isProviderName, modelFor, providerLimits, providerNames } from '../../motor/tomada/registro.ts'
...
test('modo readonly nao protege nada no kimi, e o motor recusa o papel em vez de fingir', async () => {
  const { recusaPorLimite } = await import('../../motor/euclides/tesouro/confianca.ts')
...
test('o catalogo de modos anuncia so o que o CLI aceita em execucao unica', async () => {
  const { KIMI_MODOS } = await import('../../motor/tomada/harness/kimi.ts')

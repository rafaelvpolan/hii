...
const C = await import('../../motor/agentes/clarice/doc-updater.ts')

test('mudanca em export publico afeta contrato', () => {
  const v = C.contratoPublicoMudou({
    arquivos: ['motor/cascudo/acervo.ts'],
...
test('mudanca so de corpo de funcao NAO afeta contrato — doc nao precisa mexer', () => {
  const v = C.contratoPublicoMudou({ arquivos: ['motor/cascudo/acervo.ts'], diff: '+  const x = 1\n-  const x = 0' })
...
test('quando afeta, sugere QUAIS docs — apontar "atualize a doc" sem dizer onde nao ajuda', () => {
  const v = C.contratoPublicoMudou({ arquivos: ['motor/cascudo/acervo.ts'], diff: '+export function novo() {}' })
...
test('a decisao e deterministica e nao chama modelo', async () => {
  const fonte = await lerArquivo('motor/agentes/clarice/doc-updater.ts')

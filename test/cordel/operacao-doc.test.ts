...
import { COMMANDS, ALIASES } from '../../motor/mirante/sessao.ts'
import { NOMES_DE_COMANDO_MANUAL } from '../../motor/mirante/comandos-manuais.ts'
...
test('todo agente citado como executor existe no menu do implement ou nos passos', async () => {
  const { AGENTES_IMPLEMENT, AGENTE_PADRAO } = await import('../../motor/ciclo/agente.ts')
...
test('os exemplos da tabela de perfis sao medidos, e continuam medindo o mesmo', async () => {
  const { planSteps } = await import('../../motor/oswaldo/rota/perfil.ts')
  const { DEFAULT_STEPS } = await import('../../motor/niemeyer/config.ts')

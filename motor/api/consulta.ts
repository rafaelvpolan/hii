...
interface Consulta { versao: 1; id: string; repo: string; estado: 'running' | 'succeeded' | 'failed' | 'unknown'; atividade: string; resposta: string; custoUsd: number | null; dono: number; inicio: string }
...
export function criarConsulta(b: Objeto, executar: typeof runProvider = runProvider): RespostaApi {
  ...
  const c: Consulta = { versao: 1, id, repo, atividade, estado: 'running', resposta: '', custoUsd: null, dono: process.pid, inicio: new Date().toISOString() }
  ...
  setImmediate(() => {
    void dentro(atividade, async () => {
      try {
        const r = await executar('', provider, req, 'verify')

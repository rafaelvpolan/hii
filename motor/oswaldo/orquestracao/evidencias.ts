...
export async function coletarEvidencias(plano: PlanoDeExecucao, revisao: number, wt: string, executar: typeof run = run, microtask?: string): Promise<RelatorioDeEvidencias> {
  validarPlano(plano)
  const etapa = microtask ? plano.microtasks.find(m => m.id === microtask) : undefined
  if (microtask && !etapa) throw new Error('microtask ausente do plano')
  const criterios = etapa ? plano.criterios.filter(c => etapa.criterios.includes(c.id)) : plano.criterios
  ...
  try {
    ...
    if (!microtask) writeFileAtomic(arquivoDeEvidencias(plano.id, revisao), JSON.stringify(relatorio, null, 2) + '\n')

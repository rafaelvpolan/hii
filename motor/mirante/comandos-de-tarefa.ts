import { readCard, listRepos, repoRegistered } from '../cordel/store.ts'
...
export function criarTarefa(titulo: string, repo: string): ResultadoDeAcao {
  ...
  const execucao = prepararExecucao(repo, '', texto)
  const id = core.submit({ title: texto, repo, ...execucao })
  registrarPedido(execucao.sessao_id, id, execucao.motor_modo, texto)
  const r = core.approvePlan(id)

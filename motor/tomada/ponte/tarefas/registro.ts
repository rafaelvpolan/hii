...
export function taskSyncName(): string {
  return process.env.HII_TASK_SYNC || 'none'
}
...
// a fonte da VALIDACAO: HII_TASK_SYNC com erro de digitacao caia em `null`, o
...
export function taskSyncInvalido(): string {
  ...
  return `HII_TASK_SYNC="${nome}" nao e um sync conhecido — validos: ${taskSyncNames().join(' · ')}. Nenhuma tarefa externa sera espelhada.`
}

  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (codigo: number | null, sinal: NodeJS.Signals | null) => {
      ...
      const detalhe = [stderr.join(''), stdout.join('')]
        ...
        .join('\n')
      if (!e && (codigo !== 0 || sinal)) {
        e = Object.assign(new Error(`Command failed: ${cmd}${detalhe ? `: ${detalhe}` : ''}`), { code: codigo ?? undefined, signal: sinal ?? undefined }) as ExecFileException
      } else if (e && detalhe && !e.message.includes(detalhe)) {

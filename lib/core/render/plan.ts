...
export function renderPlan(plan: Plan, opts: Partial<RenderOptions> = {}): string {
  ...
  out.push(`    hii approve ${plan.id} --plan     ${paint('aprova e enfileira', DIM, o)}`)
  out.push(`    hii plan ${plan.id} --edit        ${paint('ajusta ai/effort/steps', DIM, o)}`)
  out.push(`    hii reject ${plan.id} "motivo"    ${paint('devolve', DIM, o)}`)
  out.push('')
